...
struct BrowserCatalogTests {
    @Test func supportedDescriptorsContainExactlyTheApprovedBrowsers() {
        #expect(BrowserDescriptor.supported.map(\.bundleIdentifier) == [
            "com.apple.Safari",
            "com.google.Chrome",
    ...
    @Test func scanReturnsOnlyLocatedSupportedApplicationsInDescriptorOrder() throws {
        ...
        let localStateURL = URL(fileURLWithPath: "/home/Library/Application Support/Google/Chrome/Local State")
        ...
        #expect(discovered.map(\.application.bundleIdentifier) == [
        ...
        #expect(locator.requestedBundleIdentifiers == BrowserDescriptor.supported.map(\.bundleIdentifier))
...
private func chrome(profileID: String, profileName: String) -> DiscoveredBrowser {
    DiscoveredBrowser(
        ...
        profiles: [DiscoveredProfile(identifier: profileID, displayName: profileName, directoryURL: nil)]
    )
}
...
private func target(
    ...
    BrowserTarget(
        id: BrowserCatalog.targetID(bundleIdentifier: bundleID, profileIdentifier: profileID, mode: .normal),
        browserID: bundleID,
        label: label,
