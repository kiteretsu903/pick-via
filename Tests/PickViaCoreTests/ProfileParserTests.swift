    ...
    @Test func firefoxResolvesRelativeAndAbsolutePathsAndSkipsMalformedSections() throws {
        ...
        #expect(profiles == [
            DiscoveredProfile(
                identifier: "Personal",
                displayName: "Personal",
                directoryURL: baseDirectory.appending(path: "Profiles/personal.default-release", directoryHint: .isDirectory)
            ),
            DiscoveredProfile(
                identifier: "Work",
                displayName: "Work",
                directoryURL: URL(fileURLWithPath: "/Users/example/Firefox/Profiles/work", isDirectory: true)
            ),
        ])
    }
}
