...
For the original run, all twelve disk images were detached and the exact owned staging directory `/private/tmp/pickvia-browser-matrix.Jlmb7U` was removed after evidence and installed matches were frozen. During review remediation, the exact owned directories `/private/tmp/pickvia-browser-remediation.yNnc5z` and `/private/tmp/pickvia-inspector-repro.Kx08YQ`, including the mounted read-only images, are intentionally retained until the reproduced evidence above is committed. The contract's unique per-run fixture directories were removed by its `EXIT` trap. All installed browser apps remain in `/Applications`.
