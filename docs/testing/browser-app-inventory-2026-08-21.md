...
- RED: `/private/tmp/pickvia-inspector-contract.sh` failed because the production script did not exist.
- GREEN: the unchanged contract passed after implementation.
- Rejection cases: non-`.app` path, missing declared executable, executable without arm64, and unsigned bundle.
- Success output contains exactly these keys, in this order: `path`, `bundle_id`, `display_name`, `executable`, `version`, `architectures`, `team_id`, `authorities`, `sha256`.
- Read-only checks use `/usr/libexec/PlistBuddy`, `file`, `codesign -dvvv`, `codesign --verify --deep --strict`, `spctl --assess --type execute`, and `shasum -a 256`.
...
- Chrome and Brave DMGs carried disallowed `com.apple.FinderInfo` metadata on signed nested files. Gatekeeper still identified them as notarized Developer ID apps, but strict verification rejected the metadata. Disposable staged copies were stripped of extended attributes; executable SHA-256 values remained byte-for-byte identical, after which the unchanged inspector passed. The installed apps were copied from those exact normalized staged payloads.
- The three pre-existing Chrome Stable, Chrome Beta, and Brave Stable bundles had the same post-install Finder metadata condition. Their installed executable hashes were matched exactly to disposable normalized copies before identity inspection; the installed bundles were not modified.
...
All twelve disk images were detached. The exact owned staging directory `/private/tmp/pickvia-browser-matrix.Jlmb7U` was removed after evidence and installed matches were frozen. All installed browser apps remain in `/Applications`.
