    ...
    private var discoveryStep: some View {
        VStack(alignment: .leading, spacing: 14) {
            Label("Discover browsers", systemImage: "sparkle.magnifyingglass")
                .font(.title2.bold())
            Text("PickVia found \(model.browsers.filter(\.isAvailable).count) supported browsers on this Mac.")
...
struct DefaultStatusRows: View {
    ...
    private func statusRow(_ scheme: String, _ status: SchemeStatus) -> some View {
        GridRow {
            ...
            Label(statusText(status), systemImage: status == .isDefault ? "checkmark.circle.fill" : "exclamationmark.circle")
