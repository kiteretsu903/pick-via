...
public struct BrowserSettingsView: View {
    ...
    public var body: some View {
        List {
                    ...
                    if targets.isEmpty {
                        ...
                        ForEach(targets) { target in
                            TargetSettingsRow(
                                target: target,
                                browser: browser,
                                onRemove: target.origin == .manual ? { try? model.removeManualTarget(id: target.id) } : nil
                            )
                        }
            ...
            if model.browsers.isEmpty {
                ContentUnavailableView("No Supported Browsers", systemImage: "globe.badge.chevron.backward", description: Text("Install a supported browser, then rescan."))
            }
        }
    ...
    private func move(browserTargets: [BrowserTarget], from offsets: IndexSet, to destination: Int) {
        ...
        let globalOffsets = IndexSet(offsets.compactMap { localIndex in
        ...
        if destination < browserTargets.count {
            ...
            globalDestination = (browserTargets.last.flatMap { last in all.firstIndex { $0.id == last.id } } ?? all.count - 1) + 1
        }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack {
                ...
                Button("Add") { add() }
                    ...
                    .disabled(label.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || selectedBrowser == nil || (selectedBrowser?.family != .safari && profileIdentifier.isEmpty))
            }
        }
        .padding(24)
        .frame(width: 440)
